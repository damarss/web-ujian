...
<main>
	...
	<div class="submit">
		<div id="time"></div>
		<form action="/kuis/soal" method="POST">
			<button id="akhiri" type="submit" class="btn cons">Akhiri Kuis</button>
		</form>
	</div>
</main>
...
<script>
	...
	function loadSoal() {
		...
		const url = "https://web-ujian.herokuapp.com/get-soal-kuis?id_kuis=<%=session.getAttribute("idKuis") %>&nomor=" + nomor;
		fetch(url)
				.then(async (response) => {
				...
				.then((myJSON) => {
					...
					if (first) {
						startTimer(currentSoal.id_kuis.waktuPengerjaan * 60, display);
						setTimeout(function () {
							window.onbeforeunload = function () {};
							alert("Waktu pengerjaan berakhir");
							document.getElementById("akhiri").click();
						}, currentSoal.id_kuis.waktuPengerjaan * 60000)
						for (i = 1; i <= currentSoal.id_kuis.jumlahSoal; i++) {
	...
	function saveJawaban(jawaban) {
		fetch("https://web-ujian.herokuapp.com/save-jawaban-kuis", {
			method: "POST",
			body: JSON.stringify({
				jawabanUser: jawaban,
				id_soal: currentSoal,
	...
	function next() {
		if (currentSoal.id_kuis != null) {
			...
			nomor++;
		} else if (currentSoal.id_kuis != null) {
			if (nomor >= currentSoal.id_kuis.jumlahSoal) {
				return;
			}
			nomor++;
